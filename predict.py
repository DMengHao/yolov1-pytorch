...
# model.load_state_dict(torch.load("./weights/YOLOv1_VOC_DDP.pth"),strict=False) # 权重参数
model.load_state_dict(torch.load("./weights/YOLOv1_VOC_DDP.pth")) # 权重参数
model.eval()
confident = 0.2
iou_con = 0.4
...
class Pred():
    ...
    def result(self, img_path):
        ...
        Result = self.model(im.to('cuda'))   # 1*7*7*30
